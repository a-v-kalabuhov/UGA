...
type
  TSegmentsArrangement = (
    ...
    saCommonPart,    // есть обща€ часть
    saEqual,         // отрезки эквивалентны
    saIntersection,  // есть точка пересечени€
    saTouch,         // один конец отрезка ј лежит на отрезке B
    saConnect        // отрезки соедин€ютс€ одним из концов
  );
...
type
  IAngle = interface
    ...
    // ¬озвращает значение между 0 и 2Pi.
    function Normalize: Double;
    // ¬озвращает значение между -Pi и Pi.
...
begin
  ...
  saCollinear :
    begin
      // когда провер€ли на совпадение, то вы€снили,
      // что нет ни совпадени€ ни соединени€
      // значит провер€ем по полной:
      ...
      if D >= AngleDelta then
        Exit;
      // 2. точки сортируем по рассто€нию от начала координат
      ...
      Exit;
      { метод 2
      // 2. повернуть все точки в пр€мую ’
...
begin
  ...
  if AreEqualOrConnecting() then
    Exit;
  // есть ли обща€ часть, если отрезки лежат на одной пр€мой?
...
begin
  Result := False;
  // ќтрезок ј1¬1 вертикальный
  if Abs(A1.x - B1.x) < aDelta then
  begin
    // ќтрезок ј2¬2 горизонтальный
  ...
  // ќтрезок ј1¬1 горизонтальный
  if Abs(A1.y - B1.y) < aDelta then
  begin
    // ќтрезок ј2¬2 вертикальный
