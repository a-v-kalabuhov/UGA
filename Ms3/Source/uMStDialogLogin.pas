...
type
  TLogonEvent = function (const PeopleId: Integer;
    const UserName, RoleName, Password: String): Boolean of object;
  TLastUserEvent = procedure (var OfficeId, UserID: Integer) of object;
	...
	private
    FOnLogon: TLogonEvent;
    FOnReadLasUser: TLastUserEvent;
    ...
    function GetCurrentInitialName: String;
    function GetCurrentOfficeId: Integer;
    property OnLogon: TLogonEvent read FOnLogon write FOnLogon;
    property OnReadLastUser: TLastUserEvent read FOnReadLasUser write FOnReadLasUser;
	end;
...
resourcestring
  SQ_USER_PARAMS = 'SELECT ID, FULL_NAME, USER_NAME, ROLE_NAME, OFFICES_ID, INITIAL_NAME '
                 + 'FROM PEOPLE '
...
function TRegForm.GetCurrentFullName: String;
begin
  Result := ibqPeople.FieldByName(SF_FULL_NAME).AsString;
end;

function TRegForm.GetCurrentInitialName: String;
begin
  Result := ibqPeople.FieldByName(SF_INITIAL_NAME).AsString;
end;
