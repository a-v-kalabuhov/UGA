...
type
  TUnzipOverwriteFileEvent = procedure(const FileName: String; var Overwrite: Boolean) of object;
  TUnzipFileEvent = procedure(const FileName: String; var Accept: Boolean) of object;
  ...
  public
    ...
    function UnzipToDir(const Dir: String): Integer;
...
procedure AddFile(aZip: TZipFile; const SourceFile: String);
procedure ZipFile(const SourceFile: String; const TargetFile: string = '');
procedure UnzipFile(const SourceFile, TargetDir: String);
...
begin
  ...
  try
    ...
    for I := 0 to Pred(Count) do
      with TFileStream.Create(TargetDir + Name[I], fmCreate) do
      try
        S := Uncompressed[I];
        Write(S[1], Length(S));
      finally
...
function TZipFile.IndexOfName(const aName: String): Integer;
var
  I: Integer;
begin
  for I := 0 to Pred(Count) do
...
function TZipFile.UnzipToDir(const Dir: String): Integer;
var
  I: Integer;
  S: String;
...
procedure TZipFile.SetOnOverwriteFile(
  const Value: TUnzipOverwriteFileEvent);
begin
