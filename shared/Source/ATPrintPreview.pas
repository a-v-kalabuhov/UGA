...
type
  TFormATPreview = class(TForm)
    ...
    procedure panFitClick(Sender: TObject);
  private
...
var
  //Captions:
  MsgPreviewDlgCaption: string = 'Подготовка печати';
  MsgPreviewDlgPrintSetup: string = 'Выбор принтера...';
  MsgPreviewDlgPrint: string = 'Печать';
  MsgPreviewDlgCancel: string = 'Закрыть';
  MsgPreviewDlgOptions: string = ' Параметры листа ';
  MsgPreviewDlgPosition: string = ' Положение ';
  MsgPreviewDlgPreview: string = ' Результат ';
  MsgPreviewDlgMargins: string = ' Поля ';
  MsgPreviewDlgGamma: string = ' Яркость ';

  MsgPreviewDlgMarginLeft: string = '&Левое:';
  MsgPreviewDlgMarginTop: string = '&Верхнее:';
  MsgPreviewDlgMarginRight: string = '&Правое:';
  MsgPreviewDlgMarginBottom: string = '&Нижнее:';
  MsgPreviewDlgOptFitMode: string = '&Положение на листе:';
  MsgPreviewDlgOptFitSizeX: string = '&Ширина:';
  MsgPreviewDlgOptFitSizeY: string = '&Высота:';
...
begin
  with TFormATPreview.Create(nil) do
    try
      ...
      FFooterAtTop := AOptions.OptFooter.AtTop;
...
var
  LogPx: Integer;
begin
  LogPx := GetDeviceCaps(Printer.Handle, LOGPIXELSY);
  Result := Trunc(Printer.PageHeight / ( LogPx / APixelsPerInch));
end;
...
procedure TFormATPreview.panFitClick(Sender: TObject);
begin
