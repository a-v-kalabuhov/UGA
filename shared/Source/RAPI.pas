...
//       Converted to Delphi by Scott Crossen
//       If you make changes to this file, please e-mail me so that I can add the
//       changes to this file. - mailto:scottc@hotmail.com   !No longer valid!
//       I release this file to the public domain
//
//       Updated and fixed on 11/05/04 by Peter Stanley
//       Please email changes and updates to:
//       gothi@ps2savetools.com
//
//       Fixes 11/05/04:
//       CeRapiInitEx now works
//       CeRapiUninit now works
// --------------------------------------------------------------------------
...
TCeOIContainerStruct = record   //does not map to rapi.h type -- made just for EnumDB sample app
  OID: CeOID;
  OIDInfo: TCeOIDInfo;
end;
...
{TCeReadRecordProps     =function(hDbase:THandle; dwFlags:DWORD; cPropID :LongInt;
        var rgPropID :PCe_PropID_array;Buffer:Pointer; cbBuffer:LongInt):CeOID stdcall;}
...
TCeFindAllDatabases   =function(dwDbaseType:DWORD; wFlags:WORD; var cFindData:DWORD; var ppFindData:PCeDB_File_Data_Array):BOOL stdcall;
...
begin
  ...
  {Load ADOCE}
  AdoCEModule := LoadLibrary('C:\Program Files\Microsoft ActiveSync\adofiltr.dll');
...
begin
  ...
  if @mCeOpenDatabase <>   nil then
        Result := mCeOpenDatabase       (poid,lpszName, propid,
              dwFlags, hwndNotify)
  else
        Result := $FFFF;
end;
...
begin
  ...
  if @mCeSetFilePointer <>   nil then
        Result := mCeSetFilePointer   (hFile, DistanceToMove, DistanceToMoveHigh,
        dwMoveMethod)
  else
        Result := $FFFF;
end;
...
begin
  ...
  if @mCeRegQueryValueEx <>   nil then
        Result := mCeRegQueryValueEx (hKey, ValueName, Reserved,pType,
        pData,cbData)
  else
        Result := $FFFF;
end;
...
begin
  ...
  if @mCeRegSetValueEx <>   nil then
        Result := mCeRegSetValueEx     (hKey,ValueName,reserved,
        dwType,pData,cbData)
  else
        Result := $FFFF;
end;
...
begin
  ...
  if @mCeGetFileTime <>   nil then
        Result := mCeGetFileTime( hFile,lpCreationTime,
...
begin
  ...
  if @mCeSetFileTime <>   nil then
        Result := mCeSetFileTime         (hFile,CreationTime,
