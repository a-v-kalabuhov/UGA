...
{$endif}
    procedure DefineDateTimeVariable(AVarName: string; AValue: PDateTime);
...
{$IFDEF NAN}
  ...
  begin
    I := 0;
    while (I < LastRec1^.ExprWord.MaxFunctionArg) and
        ((LastRec1^.ArgTypes[I] <> vtFloat) or not IsNan(PDouble(LastRec1^.Args[I])^)) do
      Inc(I);
    Result := (I < LastRec1^.ExprWord.MaxFunctionArg) and
      (@LastRec1^.ExprWord.ExprFunc <> @_isNaN) and
      (@LastRec1^.ExprWord.ExprFunc <> @_assign);
  end;
{$ENDIF}
...
{$IFDEF NAN}
        if (ResType = vtFloat) and HasNaN then
          PDouble(Res.MemoryPos^)^ := Nan
        else
{$ENDIF}
          Oper(TempRec);
...
procedure TCustomExpressionParser.DefineDateTimeVariable(AVarName: string; AValue: PDateTime);
begin
