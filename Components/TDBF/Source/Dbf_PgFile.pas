...
type
  ...
  public
    ...
    function  LockFile(const Wait: Boolean): Boolean;
...
begin
  // close physical file
  if FFileLocked then UnlockFile;
  CloseFile;
...
begin
  ...
  Failed := false;
  repeat
    Result := Windows.LockFile(TFileStream(FStream).Handle, Offset, 0, Length, 0);
...
function TPagedFile.LockFile(const Wait: Boolean): Boolean;
var
  Offset: Cardinal;
  Length: Cardinal;
begin
...
procedure TPagedFile.UnlockFile;
var
  Offset: Cardinal;
  Length: Cardinal;
begin
  ...
  begin
    ...
    // FNeedLocks => FStream is of type TFileStream
    FFileLocked := not Windows.UnlockFile(TFileStream(FStream).Handle, Offset, 0, Length, 0);
  end;
end;
...
begin
  ...
  begin
    ...
    // unlock requested section
    // FNeedLocks => FStream is of type TFileStream
    Windows.UnlockFile(TFileStream(FStream).Handle, Offset, 0, Length, 0);
  end;
end;
