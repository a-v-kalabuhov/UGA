...
type
  ...
  TDbfCompareKeyEvent = function(Key1, Key2: PChar): Integer of object;
...
//    FTemporary: Boolean;          // added at runtime
  ...
  published
    property IndexFile: string read FIndexName write FIndexName;
    property SortField: string read FSortField write FSortField;
    property Options: TIndexOptions read FOptions write FOptions;
//    property Temporary: TDbfIndexDef read FTemporary write FTemporary;
  end;
...
{$endif}
  ...
  public
    ...
    function FindNearest(ARecNo: Integer; Key: PChar): Integer;
    function PhysicalRecNo: Integer;
    function MatchKey(Buffer: PChar): Integer;
...
{$endif}
  protected
    ...
    FLocaleID: LCID;
    FCompareKeys: TDbfCompareKeyEvent;
...
//    function  GetIndexCount Integer;
  ...
  public
    ...
    function  MatchKey(Buffer: PChar): Boolean;
...
type
  ...
  rMdxTag = record
    ...
    case Integer of                // MDX header version
      2:                           // dBase 4
        (
         Reserved_Ver2_0 : Byte;         // 19
         KeyType_Ver2    : Char;         // 20     C : Character
                                         //        N : Numerical
                                         //        D : Date
        );
      3:
        (                          // dBase 7
         Reserved_Ver3_0 : array [19..40] of Byte;
         KeyType_Ver3    : Char;         // 41     C : Character
                                         //        N : Numerical
                                         //        D : Date
        );
  end;
...
begin
  ...
  begin
    ...
    begin
      // we just removed the last on this page.
      EntryNo := FHighIndex;
      keyData := GetKeyData;
      if UpperPage <> nil then
        UpperPage.SetEntry(0, keyData, FPageNo);
    end;
    // check if still entries left
    if ((numEntries = 0) and (FLowerPage = nil)) or (numEntries < 0) then
      if UpperPage <> nil then
...
function TIndexPage.MatchKey(Buffer: PChar): Integer;
  // assumes Buffer <> nil
var
  keyData: PChar;
begin
  ...
  Result := FIndexFile.CompareKeys(Buffer, keyData);
end;

function TIndexPage.FindNearest(ARecNo: Integer; Key: PChar): Integer;
  // assumes Key <> nil
var
...
begin
  ...
  // if inserting and this is leaf page then
  // check if to searchkey is bigger than last entry
  if FLowerPage = nil then
  begin
    // check if no entries, then we're done quickly, add here
    if high = -1 then
    begin
      FEntryNo := 0;
      ...
      FEntryNo := high;
      FEntry := GetEntry(FEntryNo);
      // if key is bigger than last, then add entry after this one
      Result := MatchKey(Key);
      if Result > 0 then
      begin
        inc(FEntryNo);
  ...
  // define FLowIndex - 1 = neg.inf.
  // define FHighIndex = pos.inf
  // inv1: (ARecNo<>-3) -> Entry(low).Key <  Key <= Entry(high).Key
  // inv2: (ARecNo =-3) -> Entry(low).Key <= Key <  Entry(high).Key
  // vf: high - low
  while (low + 1) <> high do
  begin
  ...
  begin
    // now we may test Entry(FHighIndex) <= FLowerPage = nil
    // if low bound is moved to high bound, check if current entry matches
    if Result > 0 then
      Result := MatchKey(Key);
    // it can not be that key is bigger than all, we already tested for this!
    assert(Result <= 0);
    // test if we need to find a specific recno
    // if result < 0, then current entry is greater, so we don't have to search then
    if (Result = 0) and (ARecNo > 0) then
    begin
      // BLS to RecNo
      high := FHighIndex + 1;
      low := FEntryNo;
...
{$ifdef TDBF_INDEX_CHECK}
    ...
    // check if prior entry not greater
    if FEntryNo > 0 then
    begin
...
begin
  ...
  begin
    // get number of entries: we should be able to go past last entry
    // in leaves to add entries there
    numEntries := FHighIndex;
    if FLowerPage = nil then
      inc(numEntries);
    ...
    end else if value > numEntries then begin
      FEntryNo := numEntries + 1;
...
begin
  if EntryNo > FLowIndex then
  begin
    // if at EOF, we need to skip an entry
    if Entry = FIndexFile.EntryEof then
      EntryNo := EntryNo - 2
    else
      EntryNo := EntryNo - 1;
    if (FLowerPage <> nil) then
    ...
    Result := true;
  end else if FUpperPage<>nil then begin
...
begin
  ...
  begin
    ...
    if FLowerPage <> nil then
      FLowerPage.RecurFirst;
    Result := true;
  end else if FUpperPage<>nil then
...
begin
  ...
  begin
    ...
    begin
      ...
      FParser.ParseExpression(PIndexHdr(FIndexHeader).KeyDesc);
      // create default root
      FRoots[0] := TNdxPage.Create(Self);
      FRoot := FRoots[0];
      FSelectedIndex := 0;
      // set index locale
      FLocaleID := LCID(lcidBinary);
    end;
      ...
      begin
        ...
        begin
          ...
          if PMdxHdr(Header).Language = 0 then
            FLocaleID := lcidBinary
          else
            FLocaleID := LangId_To_Locale[PMdxHdr(Header).Language];
        end else begin
          // check if MDX - DBF language id's match
          if (PMdxHdr(Header).Language = 0) or (PMdxHdr(Header).Language = DbfLangId) then
            FLocaleID := LangId_To_Locale[DbfLangId]
          else
...
//    PMdxHdr(Header).TagsUsed := 0;
    ...
    PMdxHdr(Header).UpdYear := year - 1900;
    PMdxHdr(Header).UpdMonth := month;
    PMdxHdr(Header).UpdDay := day;
    ...
    begin
      SelectIndexVars(pos);
      PMdxTag(FMdxTag).HeaderPageNo := GetNewPageNo;
      WriteRecord(PMdxTag(FMdxTag).HeaderPageNo, FIndexHeader);
    end;
    // clear roots
    ClearRoots;
    // init vars
...
begin
  ...
  FParser.ParseExpression(FieldDesc);
  // check if result type is correct
  case FParser.ResultType of
  ...
  begin
    ...
    else if FRoots[tagNo].LowerPage <> nil then
    begin
      FRoots[tagNo].LowerPage.Free;
      FRoots[tagNo].FLowerPage := nil;
    end;
    // set leaves pointer
    ...
    FLeaf := FLeaves[tagNo];
    // create new tag
    NewMdxTag := PMdxTag(PChar(Header) + FTagOffset + tagNo * PMdxHdr(Header).TagSize);
    StrPLCopy(NewMdxTag.TagName, UpperCase(TagName), 10);
    NewMdxTag.TagName[10] := #0;
    // if expression then calculate
    NewMdxTag.KeyFormat := KeyFormat_Data;
    if ixExpression in Options then
      NewMdxTag.KeyFormat := KeyFormat_Expression;
    // what use have these reference tags?
    NewMdxTag.ForwardTag1 := 0;
    NewMdxTag.ForwardTag2 := 0;
    NewMdxTag.BackwardTag := 0;
    NewMdxTag.Reserved_Ver2_0 := 2;
    NewMdxTag.KeyType_Ver2 := fieldType;
    ...
    NewMdxTag.HeaderPageNo := FHeaderPageNo;
...
//  FIsExpression := ixExpression in Options;
  ...
  if fieldType = 'C' then
    PIndexHdr(FIndexHeader).KeyLen := FParser.ResultLen
...
begin
  ...
  begin
    ...
    begin
      // read page header
      ReadRecord(PMdxTag(PChar(Header) + FTagOffset + I * FTagSize).HeaderPageNo, FIndexHeaders[I]);
      // select it
...
{$endif}
    ...
    // store page
    MemFile.WriteRecord(Result, NewPage.PageBuffer);
  end;
  ...
  begin
    ...
    PIndexHdr(FIndexHeader).NumPages := PagesPerRecord;
    WriteHeader;
    // copy index header + root page
    idxPageNo := GetNewPageNo;
    if FIndexVersion = xBaseIV then
      PMdxTag(PChar(Header) + FTagOffset + I * FTagSize).HeaderPageNo := idxPageNo;
    // use recursive function to write all pages
    PIndexHdr(FIndexHeader).RootPage := WriteTree(FRoot);
{$ifdef TDBF_UPDATE_FIRSTLAST_NODE}
    PIndexHdr(FIndexHeader).FirstNode := PIndexHdr(FIndexHeader).RootPage;
{$endif}
    // write index header now we know the root page
    MemFile.WriteRecord(idxPageNo, FIndexHeader);
  end;

begin
  // create temporary in-memory index file
  MemFile := TPagedFile.Create(EmptyStr);
  MemFile.Mode := pfMemory;
  MemFile.AutoCreate := true;
  MemFile.OpenFile;
  MemFile.HeaderSize := HeaderSize;
  MemFile.RecordSize := RecordSize;
  MemFile.PageSize := PageSize;
  MemFile.PageOffsetByHeader := false;
  if FIndexVersion = xBaseIV then
  begin
    // reset header
    PMdxHdr(Header).NumPages := HeaderSize div PageSize;
    WriteFileHeader;
    ...
    // copy header now we know the new root pages
    DecodeDate(Now, year, month, day);
    Move(Header^, MemFile.Header^, HeaderSize);
    PMdxHdr(MemFile.Header).UpdYear := year - 1900;
    PMdxHdr(MemFile.Header).UpdMonth := month;
    PMdxHdr(MemFile.Header).UpdDay := day;
  end else begin
    // indexversion = xBaseIII
    CopySelectedIndex;
  end;
  MemFile.WriteHeader;
  // now rewrite ourselves with new content
  MemFile.WriteTo(Self);
  // free temp file
  MemFile.Free;
  // rebuild index tree(s)
  ReadIndexes;
end;
...
{$endif}
        vtFloat:
          begin
            ...
            FloatToDecimal(FloatRec, ExtValue, fvExtended, 9999, 15);
...
begin
  ...
  Result := PrepareKey(FParser.ExtractFromBuffer(Buffer), FParser.ResultType);
end;
...
procedure TIndexFile.DeleteKey;
begin
...
begin
  ...
  if StrLComp(TempBuffer, FUserKey, PIndexHdr(FIndexHeader).KeyLen) <> 0 then
  begin
...
begin
  ...
  // findnearest will not jump to EOF -> extra info = recno
  // in normal mode, do search recno so that record numbers ascending
  if (FUniqueMode = iuNormal) then
  begin
    // if inserting, search last recno matching key
    if Insert then
      searchRecNo := -3
    else
  ...
  repeat
    ...
    if Insert then TempPage.DisableRange;
    Result := TempPage.FindNearest(searchRecNo, FUserKey);
    if Insert then TempPage.UpdateBounds(TempPage.LowerPage <> nil);
    if TempPage.LowerPage = nil then
    begin
      // if item not found, try next leaf
      if (Result > 0) then
      begin
        // find first page up tree so we can advance to next item
        NextPage := TempPage;
        repeat
        ...
        begin
          // go to next item
          TempPage := NextPage;
          // disable range for searching on insert
          if Insert then TempPage.DisableRange;
...
function TIndexFile.MatchKey(Buffer: PChar): Boolean;
begin
  Result := FLeaf.MatchKey(Buffer) = 0;
end;
...
var
  ...
  procedure CheckNegZero(Key: PChar);
  begin
    if (Key[0] = #$34) and ((Byte(Key[1]) and $80) <> 0) then
    begin
      // make positive
      Key[1] := Char(Byte(Key[1]) and $7F);
    end;
  end;

begin
  // check if value is minus zero, which is nonsense
  CheckNegZero(Key1);
  CheckNegZero(Key2);
  ...
  begin
    ...
    begin
      ...
      Result := MemComp(Key1+2, Key2+2, 10);
    end else begin
      // which one is larger?
...
begin
  ...
  begin
    ...
    Result := CompareString(FLocaleID, 0, Key1, KeyLen, Key2, KeyLen);
    if Result > 0 then
      Dec(Result, 2);
  end
end;
...
begin
  ...
  pName := PChar(AIndexName);
  Result := -1;
  I := 0;
  while (I < PMdxHdr(Header).TagsUsed) and (Result < 0) do
  begin
    if StrIComp(pName, PMdxTag(PChar(Header) + FTagOffset + I * FTagSize).TagName) = 0 then
      Result := I;
    inc(I);
  end;
end;
...
procedure TIndexFile.SelectIndex(AIndex: Integer);
  // *) assumes index is in range!
begin
  // select variables
  SelectIndexVars(AIndex);
  // parse index expression; second parameter specifies a expression index
  FParser.ParseExpression(PIndexHdr(FIndexHeader).KeyDesc);
  // looked up index expression: now we can edit
  FCanEdit := not FForceReadOnly;
end;
...
procedure TIndexFile.SelectIndexVars(AIndex: Integer);
  // *) assumes all that is assumed by selectindex
  // *) WARNING: cannot find/insert/delete with this selection only
begin
  if AIndex >= 0 then
  begin
    // get pointer to index header
    ...
    if FIndexVersion = xBaseIV then
    begin
      FMdxTag := PChar(Header) + FTagOffset + AIndex * FTagSize;
      FIndexName := PMdxTag(FMdxTag).TagName;
      FHeaderPageNo := PMdxTag(FMdxTag).HeaderPageNo;
      // does dBase actually use this flag?
//      FIsExpression := PMdxTag(FMdxTag).KeyFormat = KeyFormat_Expression;
    end else begin
...
begin
  ...
  begin
    AList.Clear;
    for I := 0 to PMdxHdr(Header).TagsUsed - 1 do
      AList.Add(PMdxTag(PChar(Header) + FTagOffset + I * FTagSize).TagName);
  end;
end;
...
begin
  ...
  IndexDef.SortField := PIndexHdr(FIndexHeader).KeyDesc;
  IndexDef.Options := [];
  if FIsDescending then
...
begin
//  WriteBlock(FIndexHeaders[AIndex], RecordSize, PMdxTag(PChar(Header) + FTagOffset +
//    AIndex * FTagSize).HeaderPageNo * PageSize);
  WriteRecord(PMdxTag(PChar(Header) + FTagOffset + AIndex * FTagSize).HeaderPageNo,
    FIndexHeaders[AIndex]);
...
begin
  ...
  begin
    ...
    FSortField := TDbfIndexDef(Source).SortField;
