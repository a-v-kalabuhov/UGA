...
uses
  ...
  EzLib, fEditDB, EzBasicCtrls, EzBase, StdCtrls, Db, ADODB,
  Mask, EzThematics, EzUtils,
  EzMiscelCtrls, fAccuDraw, fAddPlace,
  fBrowse, fAccuSnap, fViews, fLegend, fRounded, fMultipart,
  EzFlashFilerGIS, ffllbase, ffdbbase, ffdb, ffllcomm, fflllgcy, ffllcomp,
  fflleng, ffsrintm, ffclreng, ffsreng, dmComms;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure Updatecached1Click(Sender: TObject);
    ...
    procedure EzFlashFilerGis1AfterPaintLayer(Sender: TObject;
      Layer: TEzBaseLayer; Grapher: TEzGrapher; var EntList: TEzEntityList;
      var AutoFree: Boolean);
    procedure EzFlashFilerGis1BeforePaintEntity(Sender: TObject;
    ...
    procedure EzFlashFilerGis1BeforePaintLayer(Sender: TObject;
      Layer: TEzBaseLayer; Grapher: TEzGrapher; var CanShow,
      WasFiltered: Boolean; var EntList: TEzEntityList;
      var AutoFree: Boolean);
    procedure EzFlashFilerGis1FileNameChange(Sender: TObject);
    procedure EzFlashFilerGis1GisTimer(Sender: TObject;
      var CancelPainting: Boolean);
    procedure EzFlashFilerGis1PrintBegin(Sender: TObject);
    procedure EzFlashFilerGis1PrintEnd(Sender: TObject);
    procedure EzFlashFilerGis1PrintProgress(Sender: TObject;
      Percent: Integer; var Cancel: Boolean);
    procedure EzFlashFilerGis1Progress(Sender: TObject;
      Stage: TEzProgressStage; const Caption: String; Min, Max,
      Position: Integer);
  private
...
uses EzActionLaunch,
  Math, ezentities, ezsystem, EzPolyClip, Inifiles, fLayers, ezrtree,
  ...
  fPref, fDBInspector, fGisInspector, fLayerInspector, fDGNImport, fDxfImport,
  fAccuDrawSetts, fAccuSnapSetts, fExpressBuilder,
...
begin
  ...
  Gis1.FlashFilerDatabase:= DataModule1.ffDatabase1;

  // the following is just for dynamic path

  // TBrowseForFolder is defined in ezinspect.pas
  With TBrowseForFolder.Create( Nil ) Do
    Try
      Flags := [bf_ReturnOnlyFSDIRS];
      Root := bl_STANDART;
      Caption := 'Select Folder';
      FolderName := FLastUsedPath;
      If Execute Then
      begin
        FLastUsedPath := AddSlash(FolderName);
      end else Exit;
    Finally
      free;
    End;
  ...
  with DataModule1 do
  begin
    ffDatabase1.Connected:= False;
    ffDatabase1.AliasName:= FLastUsedPath;
  ...
  Gis1.CreateNew( FLastUsedPath );
  Gis1.FileName:= FLastUsedPath;
  Gis1.Open;
  LoadNamedViews;
end;
...
begin
  ...
  if Gis1.Active then Gis1.Close;

  Gis1.FlashFilerDatabase:= DataModule1.ffDatabase1;

  // the following is for dynamic path changes
  With TBrowseForFolder.Create( Nil ) Do
    Try
      Flags := [bf_ReturnOnlyFSDIRS];
      Root := bl_STANDART;
...
begin
  ...
  { this causes to cache to memory the spatial r-tree index }
  //Gis1.CachedSpatialIndex:= True;
  ...
  try
    FLastUsedPath:= Inifile.ReadString('General','LastPath',ExtractFilePath(Application.Exename));
...
begin
  ShowMessage(' Please restructure with a FlashFiler utility or your own code' + #13 +
              ' You must left intact the fields UID, DELETED, XMIN, YMIN, XMAX, YMAX' + #13 +
              ', SHAPETYPE, GEOMETRY' );
end;
...
{$IFDEF FALSE}
    Inifile:= TInifile.Create(ExtractFilePath(Application.Exename)+'MapConfig.ini');
    try
      Inifile.WriteString('General', 'LastPath', FLastUsedPath);
      Filename:=IniFile.ReadString('RecentFiles', 'LastWorkedMap', '');
      if Length(Filename) > 0 then
      begin
        { check if the database exists }
        Gis1.FlashFilerDatabase.Connected:=False;
        Gis1.FlashFilerDatabase.DatabaseName:= ExtractFilepath(Filename);
        Gis1.FlashFilerDatabase.Connected:=true;
        Exists:= Gis1.TableExists( 'map_header' );
        if Exists then
        begin
...
procedure TForm1.Updatecached1Click(Sender: TObject);
begin
  Gis1.UpdateCachedTree;
end;
...
begin
  if Length(Gis1.FileName) > 0 then
    FNamedViews.LoadFromFile( ChangeFileExt(Gis1.FileName + 'views','1.vws') );
end;

procedure TForm1.SaveNamedViews;
begin
  if Length(Gis1.FileName) > 0 then
    FNamedViews.SaveToFile( ChangeFileExt(Gis1.FileName + 'views','.vws') );
end;
...
procedure TForm1.EzFlashFilerGis1AfterPaintLayer(Sender: TObject;
  Layer: TEzBaseLayer; Grapher: TEzGrapher; var EntList: TEzEntityList;
  var AutoFree: Boolean);
begin
  If ThematicBuilder1.Showthematic And (AnsiCompareText( ThematicBuilder1.LayerName, Layer.Name) = 0) then
    ThematicBuilder1.UnPrepare( Layer );
end;

procedure TForm1.EzFlashFilerGis1BeforePaintEntity(Sender: TObject;
  Layer: TEzBaseLayer; Recno: Integer; Entity: TEzEntity;
  Grapher: TEzGrapher; Canvas: TCanvas; const Clip: TEzRect;
...
procedure TForm1.EzFlashFilerGis1BeforePaintLayer(Sender: TObject;
  Layer: TEzBaseLayer; Grapher: TEzGrapher; var CanShow,
...
procedure TForm1.EzFlashFilerGis1FileNameChange(Sender: TObject);
begin
  Caption:= Format('TEzFlashFilerGIS Main Demo - %s', [GIS1.FileName]);
  if Gis1.Active And GIS1.MapInfo.IsValid then
    LayerGridBox1.Gis:= Self.Gis1;
end;

procedure TForm1.EzFlashFilerGis1GisTimer(Sender: TObject;
  var CancelPainting: Boolean);
begin
  CancelPainting:= DetectCancelPaint( DrawBox1 );
end;

procedure TForm1.EzFlashFilerGis1PrintBegin(Sender: TObject);
begin
...
procedure TForm1.EzFlashFilerGis1PrintEnd(Sender: TObject);
begin
  frmPrintProgress.Free;
end;

procedure TForm1.EzFlashFilerGis1PrintProgress(Sender: TObject;
  Percent: Integer; var Cancel: Boolean);
begin
...
procedure TForm1.EzFlashFilerGis1Progress(Sender: TObject;
  Stage: TEzProgressStage; const Caption: String; Min, Max,
  Position: Integer);
begin
   Application.ProcessMessages;
