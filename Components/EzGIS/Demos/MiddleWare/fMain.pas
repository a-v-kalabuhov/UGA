...
uses
  ...
  EzLib, Db, Grids, fViews, fBrowse, DBGrids, finspector, EzInspect, EzNumEd,
  ImgList, EzUtils, fAccuDraw, fAddPlace, fAccuSnap, fRounded,
  fMultipart, fAutoLabel, fClipboard, fClone, fLegend, EzActionLaunch,
  EzIndyGIS, IdBaseComponent, IdComponent, IdTCPConnection, IdTCPClient;
...
type
  ...
  TForm1 = class(TForm)
    ...
    GIS1: TEzIndyClientGIS;
    ScaleBar1: TEzScaleBar;
    Disconnect1: TMenuItem;
    UpdatefromServer1: TMenuItem;
    IdTCPClient1: TIdTCPClient;
    N28: TMenuItem;
    LoadClient1: TMenuItem;
    LoadAll1: TMenuItem;
    ServerServices1: TMenuItem;
    Dialogs1: TMenuItem;
    ...
    procedure UpdatefromServer1Click(Sender: TObject);
    ...
    procedure LoadClient1Click(Sender: TObject);
...
uses Math,
  ...
  Chart, Series, TeEngine, fDGNImport, fDxfImport, 
  EzDGNLayer, fAccuDrawSetts, fAccuSnapSetts, fExpressBuilder,
  fVectorialExpr, fSpatialQuery, fDGNExport, EzScrLex, EzScryacc, fClient
{$IFDEF ER_MAPPER}
  , Registry, EzERMapper
{$ENDIF}
  ;
...
begin
  ...
  If GIS1.ClientBuffered And Not DirectoryExists(RemoveSlash(GIS1.LayersSubDir)) Then
  Begin
    ShowMessage('In order for this demo to work, the GIS1.LayersSubDir property must' + #13#10 +
      'be set to a valid subdirectory');
  End;
      ...
      3:  // AutoCAD .DXF files
        begin
          ...
          try
            ...
            DxfImport.ExplodeBlocks:= Rslt=mrOk;
...
begin
  If IdTCPClient1.Connected then
    ...
    s:= 'Not Connected';
  Caption:= Format('EzGIS Indy MiddleWare Client Demo - %s', [s]);
...
begin
  ...
  CmdLine1.Clear;
  GIS1.SaveIfModified;
  with TfrmClient.create(Nil) do
    try
      Enter( Self.IdTCPClient1 );
    finally
      Free;
    end;
  If IdTCPClient1.Connected then
  ...
  with TOpenDialog.Create(Nil) do
    try
      ...
      begin
        ...
        Exit;   // Most probably: "Cannot load algorithm" error message
      end;
...
begin
  //IdTCPClient1.Disconnect;  // disconnect and continue working with the local copy of the map
  GIS1.Disconnect;  // disconnect closing the map
end;

procedure TForm1.IdTCPClient1Connected(Sender: TObject);
begin
  GIS1FileNameChange(Nil);
end;
...
procedure TForm1.UpdatefromServer1Click(Sender: TObject);
begin
  If GIS1.ClientBuffered Then Exit;
  GIS1.UpdateInfoFromServer;
  GIS1.RepaintViewports;
end;
...
begin
  ...
  Bar1.Visible:= true;
  s:='';
  If AWorkMode=wmRead then s:= 'Reading...'
  else If AWorkMode=wmWrite then s:= 'Writing...';
...
procedure TForm1.LoadClient1Click(Sender: TObject);
var
  Layer: TEzBaseLayer;
begin
  Layer:= GetLayerFromDialog(Gis1);
  ...
  Screen.Cursor:= crHourglass;
  Try
    GIS1.LoadFullLayerToBuffer(Layer.Name);
  Finally
    Screen.Cursor:= crDefault;
  End;
end;

procedure TForm1.LoadAll1Click(Sender: TObject);
begin
  Screen.Cursor:= crHourglass;
  Try
    GIS1.LoadFullLayerToBuffer('');
  Finally
    Screen.Cursor:= crDefault;
  End;
end;
...
begin
  ...
  GIS1.LoadSymbolsFromServer;
end;

procedure TForm1.Load1Click(Sender: TObject);
begin
  GIS1.LoadVectorialFontsFromServer;
end;

procedure TForm1.LoadLinetypesFile1Click(Sender: TObject);
begin
  GIS1.LoadLineTypesFromServer;
end;
