...
uses
  ...
  DBISAMTb, ezimpl, EzUtils, EzMiscelCtrls, fviews, fbrowse,
  fAccuDraw, fAddPlace, fAccuSnap, fRounded, fLegend, fMultipart, fselec;
...
type
  ...
  TForm1 = class(TForm)
    ...
    N10: TMenuItem;
    GeneralPurposeQuery1: TMenuItem;
    N15: TMenuItem;
    AccuSnap2: TMenuItem;
    AccuDraw2: TMenuItem;
    ...
    procedure GeneralPurposeQuery1Click(Sender: TObject);
...
uses Math, EzActionLaunch,
  ...
  fDGNImport, fDxfImport, fAccuDrawSetts, fAccuSnapSetts,
  fExpressBuilder, fVectorialExpr, fSpatialQuery,
...
begin
  ...
  begin
    ...
    try
      ...
      { configure for DBISAM }

      DBISAMDatabase1.Connected:=false;
      DBISAMDatabase1.Directory:= ExtractFilePath( Filename );
...
{$ENDIF}
  ...
  { configure DBISAMDatabase1 }
  DBISAMDatabase1.Connected:= false;
  DBISAMDatabase1.Directory:= Path;
  DBISAMDatabase1.Connected:= true;
...
begin
  ...
  begin
    ...
    FfrmBrowse.Enter(DrawBox1, GIS1.Layers[ LayerList1.ItemIndex ] );
  end else
...
begin
  ...
  DataSet := TDBISAMTable.Create(Nil);
...
begin
  { used when restructuring a table but we RECOMMEND that you restructure
    with a more apropriate DBISAM utility.
  }

  if FileExists( SourceFileName + '.dat' ) then
    SysUtils.RenameFile( SourceFileName + '.dat', TargetFileName + '.dat');
  if FileExists( SourceFileName + '.idx' ) then
    SysUtils.RenameFile( SourceFileName + '.idx', TargetFileName + '.idx');
  if FileExists( SourceFileName + '.blb' ) then
    SysUtils.RenameFile( SourceFileName + '.blb', TargetFileName + '.blb');
...
begin
  ...
  SysUtils.DeleteFile( Filename + '.dat' );
  SysUtils.DeleteFile( Filename + '.idx' );
  SysUtils.DeleteFile( Filename + '.blb' );
end;

procedure TForm1.Provider1QueryTableExists(Sender: TObject;
  const FileName: String; var Exists: Boolean);
begin
  Exists:= FileExists( ChangeFileExt( FileName, '.dat' ) ) and
           FileExists( ChangeFileExt( FileName, '.idx' ) ) ;  // at least these files
end;
...
Begin
  ...
  Try
    ...
    Begin
      Clear;
      Add( '', 'UID', [ixPrimary] );
      { add here any other index you need ... }
    End;
    If Not Exists Then
      CreateTable;
  Finally
...
begin
  ...
  if TDBISAMTable( DataSet ).IndexName <> '' then
    TDBISAMTable( DataSet ).IndexName:= '';  // set to primary index
  if not TDBISAMTable( DataSet ).FindKey( [Recno] ) then
...
procedure TForm1.Provider1AppendRecord(Sender: TObject; Dataset: TDataSet;
  Recno: Integer);
begin
  TDBISAMTable( DataSet ).Insert;
  { the record number for to link to the graphic files .EZD, .EZX}
  TDBISAMTable( DataSet ).FieldByName( 'UID' ).AsInteger := Recno;
  TDBISAMTable( DataSet ).Post;
end;

procedure TForm1.Provider1GetRecordCount(Sender: TObject;
  DataSet: TDataSet; var RecordCount: Integer);
begin
  RecordCount := TDBISAMTable(DataSet).PhysicalRecordCount;
end;
...
begin
  ...
  TDBISAMTable(DataSet).FlushBuffers;
end;
...
begin
  { Ouch !!! }
  TDBISAMTable( DataSet ).EmptyTable;
end;
...
begin
  ...
  begin
    ...
    TrueType := true;
    Accept:= true;
    Caption:= 'Luis Alfonso Moreno Arvayo';
  end;}
{$IFDEF FALSE}
  if AnsiCompareText(Layer.Name,'STATES_')=0 then
  begin
...
procedure TForm1.GeneralPurposeQuery1Click(Sender: TObject);
var
  Layer: TEzBaseLayer;
begin
  Layer:= GetLayerFromDialog(Gis1);
