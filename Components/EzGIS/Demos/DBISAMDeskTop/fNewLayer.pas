...
begin
  ...
  begin
     DefaultEXT:= 'DAT';
     Title:= 'Copy DAT structure from';
     Options:= [ofHideReadOnly,ofPathMustExist,ofFileMustExist];
     Filter:= 'DBISAM table files (*.DAT)|*.DAT';
     try
        if not Execute then Exit;
        Dbf:= EzBaseGis.BaseTableClass.Create(Self.FGis, FileName, false, true);
        try
