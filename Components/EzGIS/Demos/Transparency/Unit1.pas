...
uses
  ...
  EzCmdLine, EzBaseGIS, EzCtrls, EzLib, Buttons, ExtCtrls, StdCtrls,
  ComCtrls, EzBasicCtrls;
...
begin
  EzGis1.Layers.Delete('tempo', true);
  EzGis1.Close;
...
begin
  inherited;
  GisView1.BeginUpdate;
  FOldWidth:= GisView1.Width;
  FOldHeight:= GisView1.Height;
end;

procedure TForm1.WMExitSizeMove(var m: TMessage);
begin
  inherited;
  if (GisView1.Width = FOldWidth) And (GisView1.Height = FOldHeight) then
    GisView1.CancelUpdate
  else
    GisView1.EndUpdate;
end;
  ...
  begin
    { This will cause that instead of reading the bitmap from a file, this is read from
      ...
      Note:
        ...
        big bitmaps by buffering straight from the hard disk without any
    ...
    TEzBandsBitmap(Entity).AlphaChannel:= StrToIntDef(Edit1.Text,0);
  end;
end;
