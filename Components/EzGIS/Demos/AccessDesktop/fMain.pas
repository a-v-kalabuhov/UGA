...
uses
  ...
  Dialogs, Menus, EzBaseGIS, EzCtrls, EzCmdLine, Math,
  Buttons, ExtCtrls, fPrnPg, Printers, Clipbrd, fAerial,
  EzLib, EzBasicCtrls, EzBase, StdCtrls, Db, Grids,
  DBGrids, finspector, ComCtrls, EzThematics, EzInspect, EzNumEd,
  ezimpl, EzUtils, ADODB, EzMiscelCtrls, fViews, fBrowse,
  fAccuDraw, fAddPlace, fAccuSnap, fLegend, fRounded, fMultipart
{ YOU NEED THIS FOR CREATING A DATABASE}
  , ADOX_TLB, ADODB_TLB
  ;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure Provider1OpenTable(Sender: TObject; const FileName: String;
      ReadWrite, Shared: Boolean; var Dataset: TDataSet;
      var AutoDispose: Boolean);
    procedure Provider1RenameTable(Sender: TObject; const SourceFileName,
      TargetFileName: String);
    procedure Provider1DropIndexFile(Sender: TObject;
      const FileName: String);
    procedure Provider1DropTable(Sender: TObject; const FileName: String);
    procedure Provider1QueryTableExists(Sender: TObject;
      const FileName: String; var Exists: Boolean);
    procedure Provider1CreateTable(Sender: TObject; const FileName: String;
      FieldList: TStrings);
    procedure Provider1SetToRecno(Sender: TObject; Dataset: TDataSet;
      Recno: Integer);
    procedure Provider1AppendRecord(Sender: TObject; Dataset: TDataSet;
      Recno: Integer);
    procedure Provider1GetRecordCount(Sender: TObject; DataSet: TDataSet;
      var RecordCount: Integer);
    procedure Provider1DeleteRecord(Sender: TObject; DataSet: TDataSet);
    procedure Provider1FlushTable(Sender: TObject; DataSet: TDataSet);
    procedure Provider1PackTable(Sender: TObject; DataSet: TDataSet);
    procedure Provider1RecallRecord(Sender: TObject; DataSet: TDataSet);
    procedure Provider1SetUseDeleted(Sender: TObject; DataSet: TDataSet;
      var IsDeleted: Boolean);
    procedure Provider1GetIsDeleted(Sender: TObject; DataSet: TDataSet;
      var IsDeleted: Boolean);
    procedure Provider1GetRecno(Sender: TObject; DataSet: TDataSet;
      var Recno: Integer);
    procedure Provider1ZapTable(Sender: TObject; DataSet: TDataSet);
    procedure GIS1GisTimer(Sender: TObject; var CancelPainting: Boolean);
    procedure GIS1BeforeOpen(Sender: TObject);
...
uses EzActionLaunch,
  Chart, Series, TeEngine, ezentities, ezsystem, EzPolyClip, Inifiles, fLayers, ezrtree,
  ezconsts, fLineTypeEd, fdemoprevw, fSymbEd, fRasterImageEd, 
  fProgr, ezshpimport, EzDxfImport, EzMIFIMport, EzDGNImport,
  fLineType, fSymbolType, fFontStyle, fLayerSelect, fMapUnits,
...
var
  ...
  //Table: _Table; // the table
  //Column1: _Column; // columns for the dummy table
...
begin
  ...
  begin
    ...
    try
      ...
      // This creates an Access 2000 database using ADOX.
      ...
      BaseName:= ChangeFileExt( FileName, '.MDB' );

      If FileExists( BaseName ) then
        SysUtils.DeleteFile( BaseName );
      ...
      Catalog:= Nil;  // dispose this COM object

      { You must create here the access database (.mdb) if that not already exists }
      ADOConnection1.ConnectionString:= 'Provider=Microsoft.Jet.OLEDB.4.0;Persist Security Info=False;' +
        Format( 'Data Source=%s',[ BaseName ]) ;
...
begin
  ...
  with TOpenDialog.Create(Self) do
    try
      ...
      If not FileExists( ChangeFileExt( FileName, '.mdb') ) then
      begin
        { create the .MDB file }
        BaseName:= ChangeFileExt( FileName, '.MDB' );
      ...
      GIS1.Open;  { In the OnBeforeOpen will try to open the .mdb file }
...
begin
  ...
  {If Not ( ezbasegis.BaseTableClass = TEzProviderTable )  then
  begin
    ShowMessage('In order to this demo to run, you need to activate optional' + #13+
                'switch compilation "DATASET_PROVIDER" in unit ezimpl.pas' + #13 +
                'Also, all other switches must be set to disabled ' );
...
begin
  ...
  begin
    ...
    with TfrmDemoBrowse.Create(Application) do
...
begin
  ShowMessage( 'We recommend that you restructure with a more apropriate DBISAM utility !' );
end;
...
begin
  ...
  ShowText:= Format('Custom Clicked on (%.*n, %.*n)',
...
{ NOTE:
  The following event are specifically for handling DBISAM databases attached
  to one layer, that is to say, one layer will have the following native files:
  ...
  And additionally, for database information, the following DBISAM files will be used

  XXX.DAT
  XXX.IDX
  XXX.BLB * (only if memo/binary/graphic fields are used
}

procedure TForm1.Provider1OpenTable(Sender: TObject;
  const FileName: String; ReadWrite, Shared: Boolean;
  var Dataset: TDataSet; var AutoDispose: Boolean);
begin

  DataSet := TADOTable.Create(Nil);
...
procedure TForm1.Provider1RenameTable(Sender: TObject;
  const SourceFileName, TargetFileName: String);
begin
  { used when restructuring a table but we RECOMMEND that you restructure
    with a more apropriate utility.
  }
  raise Exception.CreateFmt( 'I don'' know how to rename table %s', [SourceFileName] );

end;

procedure TForm1.Provider1DropIndexFile(Sender: TObject;
  const FileName: String);
begin
  { used when restructuring a table but we RECOMMEND that you restructure
    with a more apropriate utility.
...
procedure TForm1.Provider1DropTable(Sender: TObject;
  const FileName: String);
var
  sql: string;
begin
  { Mainly used when packing a layer and its corresponding table  }
  sql:= 'DROP TABLE ' + ChangeFileExt( ExtractFileName( FileName ),'' );
  ...
  //raise Exception.CreateFmt( 'Don''t know how to delete the table %s', [FileName] );
end;

procedure TForm1.Provider1QueryTableExists(Sender: TObject;
  const FileName: String; var Exists: Boolean);
begin
  Exists:= true;
  { we must return true only if table exist already . How to know ? }
end;

procedure TForm1.Provider1CreateTable(Sender: TObject;
  const FileName: String; FieldList: TStrings);
Var
  s: String;
  v: boolean;
  i: integer;
...
Begin
  AFieldList:= FieldList;  // TDataSet has also FieldList property

  TableName := ChangeFileExt( ExtractFileName( FileName ), '' );
  sql:= Format( 'CREATE TABLE [%s] (UID long CONSTRAINT PrimaryKey PRIMARY KEY, DELETED bit', [TableName] );
  ...
  Begin
    ...
    if AnsiCompareText( fs, 'deleted') = 0 then Continue;

    Case ft[1] Of
      'C':
        sql := sql + Format( ', %s varchar(%d)', [fs, fl] );
      'F', 'N':
        If fd = 0 Then
          sql := sql + Format( ', %s int', [fs] )
        Else
          sql := sql + Format( ', %s float', [fs] );
      'M':
        sql := sql + Format(', %s text', [fs]);
      'G', 'B':
        sql := sql + Format(', %s image', [fs]);
      'L':
        sql := sql + Format(', %s yesno', [fs]);
      'D':
        sql := sql + Format(', %s date', [fs]);
      'T':
        sql := sql + Format(', %s time', [fs]);
      'I':
        sql := sql + Format(', %s int', [fs]);
    End;
  End;
  ...
  With TADOCommand.Create( Nil ) Do
    Try
      Connection:= Self.ADOConnection1;
      CommandText := sql;
      Execute( RowsAffected, EmptyParam );
    Finally
      Free;
    End;
end;

procedure TForm1.Provider1SetToRecno(Sender: TObject; Dataset: TDataSet;
  Recno: Integer);
begin
  { position the table in a specific record }
  if TADOTable( DataSet ).IndexName <> '' then
    TADOTable( DataSet ).IndexName:= '';  // set to primary index
  if not TADOTable( DataSet ).Locate('UID', Recno, [] ) then
    raise Exception.Create( 'Record not found !' );
end;

procedure TForm1.Provider1AppendRecord(Sender: TObject; Dataset: TDataSet;
  Recno: Integer);
begin
  TADOTable( DataSet ).Insert;
  { the record number for to link to the graphic files .EZD, .EZX}
  TADOTable( DataSet ).FieldByName( 'UID' ).AsInteger := Recno;
  TADOTable( DataSet ).Post;
end;

procedure TForm1.Provider1GetRecordCount(Sender: TObject;
  DataSet: TDataSet; var RecordCount: Integer);
begin
  RecordCount := TADOTable( DataSet).RecordCount;
end;

procedure TForm1.Provider1DeleteRecord(Sender: TObject; DataSet: TDataSet);
begin
  with TADOTable( DataSet) do
  begin
    Edit;
...
procedure TForm1.Provider1FlushTable(Sender: TObject; DataSet: TDataSet);
begin
  if not DataSet.Active then Exit;
  TADOTable( DataSet).UpdateBatch;
end;

procedure TForm1.Provider1PackTable(Sender: TObject; DataSet: TDataSet);
begin
  DataSet.First;
...
procedure TForm1.Provider1RecallRecord(Sender: TObject; DataSet: TDataSet);
begin
  If DataSet.FieldByname( 'DELETED' ).AsBoolean Then
  Begin
    DataSet.Edit;
...
procedure TForm1.Provider1SetUseDeleted(Sender: TObject; DataSet: TDataSet;
  var IsDeleted: Boolean);
begin
  if IsDeleted then
  begin
  ...
  begin
    DataSet.Filtered:= false;
    DataSet.Filter:= 'DELETED = FALSE';
    DataSet.Filtered:= true;
  end;
end;

procedure TForm1.Provider1GetIsDeleted(Sender: TObject; DataSet: TDataSet;
  var IsDeleted: Boolean);
begin
  IsDeleted:= DataSet.FieldByName('DELETED').AsBoolean;
end;

procedure TForm1.Provider1GetRecno(Sender: TObject; DataSet: TDataSet;
  var Recno: Integer);
begin
  { uid is the fictitious physical record number and primary index field }
  Recno:= DataSet.FieldByName('uid').AsInteger;
end;

procedure TForm1.Provider1ZapTable(Sender: TObject; DataSet: TDataSet);
begin
  { Ouch !!! }
  TADOTable( DataSet ).DeleteRecords( arAll );
end;
...
procedure TForm1.GIS1BeforeOpen(Sender: TObject);
var
  Filetest: string;
begin
  { in this event it is supposed to create an Access table where the map exists (file .ezm)
    if that Access table does not already exists. In this example, only an exception is raised
    indicating that condition.
  }
  Filetest:= ChangeFileExt( GIS1.FileName, '.MDB');
  If Not FileExists( Filetest ) then
    raise Exception.CreateFmt( 'Needed Access database %s does not exists ', [Filetest] );
  { first open the access database }
  ADOConnection1.Connected:=false;
  ADOConnection1.ConnectionString:= 'Provider=Microsoft.Jet.OLEDB.4.0;Persist Security Info=False;' +
    Format( 'Data Source=%s',[ Filetest ]) ;
