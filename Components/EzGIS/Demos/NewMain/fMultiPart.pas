...
Begin
  ...
  Begin
    ...
    If Assigned( DrawBox.OnBeforeInsert ) Then
      DrawBox.OnBeforeInsert( DrawBox, TmpLayer, TmpEnt, Accept );
    Recno := 0;
    If Accept Then
      Recno:= DrawBox.AddEntity( GIS.CurrentLayerName, TmpEnt );
    { the current layer could be changed on the OnBeforeInsert event}
    TmpLayer := GIS.CurrentLayer;
    If Accept And Assigned( DrawBox.OnAfterInsert ) Then
      DrawBox.OnAfterInsert( DrawBox, TmpLayer, Recno );
    // symbols modified
