...
uses Windows, SysUtils, Classes, Graphics, Forms, Controls, StdCtrls,
  Buttons, ExtCtrls, ComCtrls, Dialogs, ezbasegis, Grids, EzMiscelCtrls,
  Menus, EzActionLaunch;
...
procedure TfrmLayers.StopPaintingThread;
begin
  if FPaintingThread = Nil then Exit;
...
procedure TfrmLayers.ThreadDone(Sender: TObject);
begin
  with FDrawBox do
  begin
    CancelUpdate;
...
procedure TfrmLayers.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  StopPaintingThread;
end;
...
begin
  { a thread is used for repainting the viewport }
  StopPaintingThread;
  with FDrawBox.ScreenBitmap do
    FDrawBox.ClearCanvas( Canvas, Rect(0,0,Width,Height), FDrawBox.Color );
  FPaintingThread := TEzPainterThread.Create(ThreadDone,
    FDrawBox.Grapher.CurrentParams.VisualWindow,
    FDrawBox, False, pmAll );
