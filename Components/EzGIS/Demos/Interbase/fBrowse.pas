...
uses
  ...
  EzBase, Ezlib, ExtCtrls, EzTable, IBCustomDataSet, IBQuery;
...
begin
  ...
  with IBQuery1 do
  begin
    Database:= fMain.Form1.IBDatabase1;
    Transaction:= fMain.Form1.IBTransaction1;
    Close;
    Unidirectional:= false;
    ParamCheck:=false;
    SQL.Text:= Format('SELECT * FROM ent_%s', [Layer.name]);
    Open;
  end;
...
begin
  StatusBar1.simpletext := Format('%d Records. %d Selected (Recno: %d)', [FLayerRecordCount,
     FDrawBox.Selection.NumSelectedInLayer(FLayer), IBQuery1.FieldByName('uid').AsInteger]);
end;
...
begin
  with DBGrid1 do
  begin
     if IBQuery1.FieldByName('deleted').AsBoolean then
     begin
        DrawField('', Rect, Canvas, Column.Font,
           Column.Alignment, [], clBlack, clRed);
     end else if (FSelLayer<>nil) and FSelLayer.IsSelected(IBQuery1.FieldByName('uid').AsInteger) then
     begin
...
begin
  Form1.DrawBox1.SetEntityInViewEx(FLayer.Name,IBQuery1.FieldByName('uid').AsInteger,Extents1.Checked);
end;
...
begin
  Form1.DrawBox1.BlinkEntityEx(Flayer.Name,IBQuery1.FieldByName('uid').AsInteger);
end;
...
begin
  ...
  begin
    Close;
    SQL.Text := Format( 'SELECT * FROM ent_%s', [CboLayers.Text] );
    Open;
  end;
end;
...
begin
  with FDrawBox do
  begin
    Selection.Toggle(FLayer, IBQuery1.FieldByName('uid').AsInteger);
    Selection.Add(FLayer, IBQuery1.FieldByName('uid').AsInteger);
    Repaint;
  end;
end;
...
var
  WasSelected: Boolean;
begin
  WasSelected := FDrawBox.Selection.IsSelected(FLayer, IBQuery1.FieldByName('uid').AsInteger);
  FDrawBox.Selection.Clear;
  if not WasSelected then
     FDrawBox.Selection.Add(FLayer, IBQuery1.FieldByName('uid').AsInteger);
  if AutoView1.Checked then
  begin
     {Show in the map only and only when one is selected}
     FDrawBox.SetEntityInViewEx(FLayer.Name, IBQuery1.FieldByName('uid').AsInteger, Extents1.Checked);
  end;
  UpdateSelectionLayer;
  FDrawBox.Repaint;
end;
