...
{***********************************************************}
{     EzGIS/CAD Components   - TEzIBGis demo                }
...
uses
  ...
  EzLib, fEditDB, EzBasicCtrls, EzBase, StdCtrls, Db,
  Mask, EzThematics, EzIBGIS, IBQuery, IBDatabase,
  IBCustomDataSet, IBUpdateSQL, IBSQL, EzUtils, EzMiscelCtrls,
  fAccuDraw, fAddPlace, fAccuSnap, fBrowse,fViews, fRounded, fLegend,
  fMultipart;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure GIS1LabelEntity(Sender: TObject; Layer: TEzBaseLayer;
      ...
      Canvas: TCanvas; const Clip: TEzRect; var Caption: String;
    ...
    procedure Gis1AfterDragDrop(Sender: TObject; Layer: TEzBaseLayer;
      Recno: Integer);
    procedure Gis1CreateIBSQL(Sender: TObject; var IBSQL: TIBSQL);
    procedure FormDestroy(Sender: TObject);
    procedure NamedViews1Click(Sender: TObject);
    procedure BtnSymbolMultClick(Sender: TObject);
    procedure AccuDraw2Click(Sender: TObject);
...
uses Math, EzActionLaunch,
  ...
  fAccuDrawSetts, fAccuSnapSetts, fExpressBuilder, fselec,
  fVectorialExpr, fSpatialQuery,
...
begin
  ...
  begin
    Connected:= false;
    DatabaseName:= SaveDialog4.FileName;
    Params.Clear;
  ...
  Gis1.CreateNew( SaveDialog4.FileName );
  Gis1.FileName:= SaveDialog4.FileName;
  Gis1.Open;
  LoadNamedViews;

  Caption:= 'TEzIBGis Demo - ' + SaveDialog4.FileName;
end;
...
begin
  ...
  DatabaseName:= OpenDialog4.FileName;
...
begin
  ...
  If Self.Gis1.MapInfo.IsValid Then
...
begin
  ShowMessage(' We recommend that you restructure the layer table with ' + #13 +
              ' more appropriate utility ' );
end;
...
procedure TForm1.GIS1LabelEntity(Sender: TObject; Layer: TEzBaseLayer;
  ...
  const Clip: TEzRect; var Caption: String; var Position: TEzLabelPos;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Gis1.Open;
        Self.Caption:= 'TEzIBGis Demo - ' + FileName;
      end;
    finally
      Inifile.Free;
    end;
...
procedure TForm1.Gis1AfterDragDrop(Sender: TObject; Layer: TEzBaseLayer;
  Recno: Integer);
begin
  ShowMessage( Format('Dragged Layer : %s, RecNo: %d', [Layer.Name, Recno] ) );
end;

procedure TForm1.Gis1CreateIBSQL(Sender: TObject; var IBSQL: TIBSQL);
begin
  IBSQL:= TIBSQL.Create(Nil);
  IBSQL.Database:= IBDatabase1;
  IBSQL.Transaction:= IBTransaction1;
end;
...
begin
  ...
  If FTestBitmap<>Nil then FTestBitmap.free;
...
procedure TForm1.AccuDraw2Click(Sender: TObject);
begin
