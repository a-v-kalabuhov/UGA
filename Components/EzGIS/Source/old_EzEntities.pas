...
Begin
  ...
  FCurvePoints.Clear;
  CurrAngle := StartAng;
  For I := 0 To FPointsInCurve - 1 Do
  Begin
    TmpAngle := CurrAngle;
...
Begin
  ...
  Begin
    M := Rotate2d( FRotangle, Point2d( CX, CY ) );
    For cnt := 0 To FCurvePoints.Count - 1 Do
...
Begin
  If (FPoints = Nil) Or (FPoints.Count = 0) Then Exit;
...
Begin
  ...
  Try
    ...
    //FFontTool.Height:= Dist2d(FPoints[0], FPoints[1]);
    UpdateExtension;
  Finally
    EndUpdate;
  End;
    ...
    Begin
      ...
      Try
        ...
        if Grapher = Nil then
          ...
          //MulDiv( BasePoints, GetDeviceCaps( dc, LOGPIXELSY ), 72 );
        ...
        Begin
          ...
          Begin
            ...
            {IntWidth := EzLib.IMax( IntWidth, TextLineWidth( Workrow ) );}
            ASize.cX := 0;
            ASize.cY := 0;
          ...
          Begin
            { IntWidth := EzLib.IMax( IntWidth, TextLineWidth( Workstr ) ); }
            ASize.cX := 0;
            ASize.cY := 0;
            Windows.GetTextExtentPoint32( dc, PChar( Workrow ), Length( Workrow ), ASize );
            IntWidth := EzLib.IMax( IntWidth, ASize.cX );
...
Begin
  ...
  // ≈сли необходимо рисуем полигон под текстом
  ...
  begin
    ...
    //TmpHeight:= MulDiv(Trunc(Grapher.GetSizeInPoints(FFontTool.Height)),Grapher.DpiY,72);
  if TmpHeight <= 1 then
  begin
  ...
  //if not Globalinfo.GlobalParams.ShowTextRotated then UseAngle := 0;
  ...
  oldFont := 0;
  if (DrawMode <> dmRubberPen) {And IsSingleLine} and (UseAngle <> 0) then
  begin
    GetObject(Canvas.Font.Handle, SizeOf(LogRec), @LogRec);
    LogRec.lfEscapement := Round(RadToDeg(UseAngle) * 10);
    //LogRec.lfOrientation:= LogRec.lfEscapement;
    LogRec.lfOutPrecision := OUT_TT_ONLY_PRECIS;
    StrPCopy(LogRec.lfFaceName, FFontTool.Name);
...
{    else
  ...
  End;
  If (DrawMode <> dmRubberPen) And (UseAngle <> 0) Then
  Begin
       ...
       begin
         ...
         PT_MOVETO :
           begin
             { start of new entity }
             V.Clear;
             V.Add(Grapher.PointToReal(APoints[iCount]));
           end;
         PT_LINETO :
           begin
             V.Add(Grapher.PointToReal(APoints[iCount]));
        ...
        for iCount := 0 to EntityList.Count-1 do
          EntityList[iCount].Draw(Grapher, Canvas, Clip, DrawMode, Data);
     finally
        V.free;
        EntityList.free;
...
const
  ...
  S_NOMENCLATURE_CHARS = 'јЅ¬√ƒ≈∆«» ЋћЌќѕ–—“”‘’÷„ЎўЁёя';
...
begin
  Result := (FNomenclature = '') or (FNomenclature = 'ќшибка');
end;
