...
type
  TEzTIFFGraphic = class(TTIFFGraphic)
  private
    FEzStream: TStream;
    FEzDecoder: TDecoder;
    FEzDeprediction: procedure(P: Pointer; Count: Cardinal);
    FEzOffsets, FEzByteCounts: TCardinalArray;
  public
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Length(RowsPerStrip) = 0) then
      begin
        SetLength(RowsPerStrip, 1);
        RowsPerStrip[0] := Height;
      end
      else
      if (RowsPerStrip[0] = $FFFFFFFF) then
      begin
        SetLength(RowsPerStrip, 1);
        RowsPerStrip[0] := Height;
      end;
...
procedure TEzTIFFGraphic.EzClose;
begin
  EzCloseFromStream;
  if Assigned(FEzStream) then
  begin
    FEzStream.Free;
    FEzStream:= nil;    // Delphi 4 does not have FreeAndNil :-(
  end;
end;

procedure TEzTIFFGraphic.EzCloseFromStream;
begin
  if Assigned(FEzDecoder) then
    FEzDecoder.DecodeEnd;
  FEzDecoder.Free;
  Finalize(FEzOffsets);
  Finalize(FEzByteCounts);
end;

procedure TEzTIFFGraphic.EzOpen(const FileName: String);
begin
  FEzStream := TFileStream.Create(FileName, fmOpenRead or fmShareDenyNone);
  EzOpenFromStream(FEzStream);
end;

procedure TEzTIFFGraphic.EzOpenFromStream(AStream: TStream);
var
  IFDCount: Word;
  ColorMap: Cardinal;
  StripSize: Cardinal;
begin
  ...
  if ReadImageProperties(AStream, 0) then
  begin
    ...
    begin
      ...
      // read data of the first image file directory (IFD)
      AStream.Position := FBasePosition + FirstIFD;
      AStream.ReadBuffer(IFDCount, SizeOf(IFDCount));
      if ioBigEndian in Options then IFDCount := Swap(IFDCount);
      SetLength(FIFD, IFDCount);
      AStream.ReadBuffer(FIFD[0], IFDCount * SizeOf(TIFDEntry));
      if ioBigEndian in Options then SwapIFD;
      SortIFD;
      ...
      // data organization
      GetValueList(AStream, TIFFTAG_STRIPOFFSETS, FEzOffsets);
      GetValueList(AStream, TIFFTAG_STRIPBYTECOUNTS, FEzByteCounts);

      // retrive additional tile data if necessary
      if ioTiled in Options then
      begin
        GetValueList(AStream, TIFFTAG_TILEOFFSETS, FEzOffsets);
        GetValueList(AStream, TIFFTAG_TILEBYTECOUNTS, FEzByteCounts);
      end;
      ...
      // now that the pixel format is set we can also set the (possibly large) image dimensions
      Self.Width := Width;
      Self.Height := RowsPerStrip[0];  { EzGis - define a bitmap height of max rows per strip }
      if (Width = 0) or (Height = 0) then GraphicExError(gesInvalidImage, ['TIF/TIFF']);
      ...
      begin
        ...
        begin
          ...
          if StripSize > 0 then
          begin
            AStream.Position := FBasePosition + ColorMap;
            // number of palette entries is also given by the color map tag
            // (3 components each (r,g,b) and two bytes per component)
            AStream.ReadBuffer(FPalette[0] , 2 * StripSize);
            Palette := ColorManager.CreateColorPalette([@FPalette[0], @FPalette[StripSize div 3],
                                                        @FPalette[2 * StripSize div 3]], pfPlane16Triple, StripSize, False);
          end;
        end
      ...
      begin
        ...
        case Predictor of
          PREDICTION_HORZ_DIFFERENCING: // currently only one prediction scheme is defined
            case SamplesPerPixel of
              4:
                FEzDeprediction := Depredict4;
              3:
                FEzDeprediction := Depredict3;
            else
              FEzDeprediction := Depredict1;
            end;
        end;
      end;
      ...
      case Compression of
        ...
        {$ifdef UseLZW}
        ctLZW:
          FEzDecoder := TTIFFLZWDecoder.Create;
        {$endif}
        ctPackedBits:
          FEzDecoder := TPackbitsRLEDecoder.Create;
        ctFaxRLE,
        ctFaxRLEW:
          FEzDecoder := TCCITTMHDecoder.Create(GetValue(TIFFTAG_GROUP3OPTIONS),
                                              ioReversed in Options,
                                              Compression = ctFaxRLEW,
                                              Width);
        ctFax3:
          FEzDecoder := TCCITTFax3Decoder.Create(GetValue(TIFFTAG_GROUP3OPTIONS), ioReversed in Options, False, Width);
        ctJPEG:
          begin
            // some extra work is needed for JPEG
            GetValueList(AStream, TIFFTAG_JPEGTABLES, JPEGTables);

            FEzDecoder := TTIFFJPEGDecoder.Create(@ImageProperties);
          end;
        ctThunderscan:
          FEzDecoder := TThunderDecoder.Create(Width);
        ctLZ77:
          FEzDecoder := TLZ77Decoder.Create(1, True);
      else
        {
        COMPRESSION_OJPEG,
        COMPRESSION_CCITTFAX4
      ...
      if Assigned(FEzDecoder) then FEzDecoder.DecodeInit;
      Progress(Self, psEnding, 0, False, FProgressRect, '');

      StripCount := Length(FEzOffsets);
...
procedure TEzTIFFGraphic.EzReadStrips(StartStrip, StopStrip: Integer);
var
  Buffer: Pointer;
  Run: PChar;
  Pixels,
...
begin
  ...
  begin
    ...
    if not ((StartStrip >= 0) and (StartStrip < StripCount)) or
       not ((StopStrip  >= 0) and (StopStrip  < StripCount)) then Exit;

    // calculate height of the bitmap
    NewHeight:= 0;
    for K:= StartStrip to StopStrip do
    begin
      if K < Length(RowsPerStrip) then
        Inc(NewHeight,RowsPerStrip[K])
      else
        Inc(NewHeight,RowsPerStrip[High(RowsPerStrip)]);
    end;
    ...
    CurrentRow := 0;
    CurrentStrip:= StartStrip;
    StripCount := Length(FEzOffsets);
    while CurrentStrip <= StopStrip do
    begin
      FEzStream.Position := FBasePosition + FEzOffsets[CurrentStrip];
      if CurrentStrip < Length(RowsPerStrip) then StripSize := BytesPerLine * RowsPerStrip[CurrentStrip]
                                             else StripSize := BytesPerLine * RowsPerStrip[High(RowsPerStrip)];
      GetMem(Buffer, StripSize);
      Run := Buffer;
      try
        ...
        if Assigned(FEzDecoder) then
        begin
          GetMem(EncodedData, FEzByteCounts[CurrentStrip]);
          try
            DataPointerCopy := EncodedData;
            FEzStream.Read(EncodedData^, FEzByteCounts[CurrentStrip]);
            // need pointer copies here because they could get modified
            // while decoding
            FEzDecoder.Decode(DataPointerCopy, Pointer(Run), FEzByteCounts[CurrentStrip], StripSize);
          finally
            if Assigned(EncodedData) then FreeMem(EncodedData);
          end;
        end
        ...
        begin
          FEzStream.Read(Buffer^, StripSize);
        end;

        Run := Buffer;
        // go for each line (row) in the strip
        while (Integer(CurrentRow) < Integer(Self.Height)) and ((Run - Buffer) < Integer(StripSize)) do
        begin
          Pixels := ScanLine[CurrentRow];
          // depredict strip if necessary
          if Assigned(FEzDeprediction) then FEzDeprediction(Run, Width - 1);
          // any color conversion comes last
          ColorManager.ConvertRow([Run], Pixels, Width, $FF);
          Inc(PChar(Run), BytesPerLine);
          Inc(CurrentRow);

          // EzGis commented this
          //Progress(Self, psRunning, MulDiv(CurrentRow, 100, Height), True, FProgressRect, '');
          //OffsetRect(FProgressRect, 0, 1);
        end;
