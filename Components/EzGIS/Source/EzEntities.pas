...
Uses
  Controls, SysUtils, Classes, Windows, Graphics, Math, Dialogs, Printers, 
...
{$ENDIF}
    Procedure DrawVector( V: TEzVector; Const PenStyle: TEzPenStyle;
      const PenScale: Double; Grapher: TEzGrapher;
...
{$ENDIF}
  ...
  Protected
    ...
    Procedure MoveAndRotateControlPts( Var MovePt, RotatePt: TEzPoint; Grapher: TEzGrapher );
    Procedure SetRotangle( Value: Double ); virtual;
    Function GetEntityID: TEzEntityID; Override;
    Function BasicInfoAsString: string; Override;
  Public
...
{$ENDIF}
  ...
  Protected
    Function GetEntityID: TEzEntityID; Override;
    Function BasicInfoAsString: string; Override;
    Procedure SetRotangle( Value: Double ); override;
  Public
  ...
  TEzBitmapRef = Class( TEzRectangle )
  Private
    FAlphaChannel: Byte;
    { assigned temporarily }
    FVisible: Boolean;
  ...
  Protected
    ...
    Procedure SetRotangle( Value: Double ); override;
  Public
    Constructor CreateEntity( Const P1, P2: TEzPoint; Bitmap: TBitmap );
    Destructor Destroy; Override;
    Procedure Initialize; Override;
    Procedure LoadFromStream( Stream: TStream ); Override;
    Procedure SaveToStream( Stream: TStream ); Override;
  ...
  Protected
    function GetBackgroundMode: Integer; virtual;
    Function GetEntityID: TEzEntityID; Override;
    Function BasicInfoAsString: string; Override;
    Function AttribsAsString: string; Override;
    function GetDrawTextFlags: Word; virtual;
    function DoMakePolyPoints: Boolean; virtual;
  Public
    Constructor CreateEntity( Const Pt: TEzPoint;
    ...
    property DontRotate: Boolean read FDontRotate write FDontRotate;
    Property FontTool: TEzFontTool Read GetFontTool ;
    Property Text: String {$IFDEF BCB} Read GetText Write SetText {$ELSE} Read FText Write FText {$ENDIF};
  End;

  TEzAlignedTTText = class(TEzTrueTypeText)
  private
    FAlignment: TAlignment;
  protected
    function GetEntityID: TEzEntityID; override;
    function BasicInfoAsString: String; override;
    function GetDrawTextFlags: Word; override;
    function DoMakePolyPoints: Boolean; override;
  public
    procedure LoadFromStream(Stream: TStream); override;
    procedure SaveToStream(Stream: TStream); override;
    function StorageSize: Integer; override;
  ...
  TEzAlignedText2 = class(TEzAlignedTTText)
  private
    FTransparent: Boolean;
  protected
    function GetBackgroundMode: Integer; override;
    function GetEntityID: TEzEntityID; override;
  public
    procedure LoadFromStream(Stream: TStream); override;
    procedure SaveToStream(Stream: TStream); override;
  ...
  TNomenclatureParamFunc = function (var aNomenclature: string; var aTop, aLeft: Integer): Boolean;
  ...
  private const
    I_MAP500_SIZE = 250; // в метрах
  private
    class var FNomenclatureFunc: TNomenclatureParamFunc;
    class procedure SetNomenclatureFunc(const Value: TNomenclatureParamFunc); static;
  protected
    function GetEntityID: TEzEntityID; override;
    procedure ReCreate;
  ...
  public
    ...
    property Text: TEzTrueTypeText read FText;
  public
    class property NomenclatureFunc: TNomenclatureParamFunc read FNomenclatureFunc write SetNomenclatureFunc;
  end;

  TEzSquareText = class(TEzRectangle)
  private
    FText: TEzTrueTypeText;
    FCaption: string;
    FSquareSize: Double;
  ...
  protected
    function GetEntityID: TEzEntityID; override;
  public
    constructor CreateEntity(const aCaption: String; const aTop, aLeft, aSize: Double);
    destructor Destroy; override;
    //
    procedure Initialize; override;
    procedure Draw(Grapher: TEzGrapher; Canvas: TCanvas; Const Clip: TEzRect;
  ...
  TEzPageEntity = class(TEzRectangle)
  private
    FText: TEzTrueTypeText;
    FNumber: String;
    FShowNumber: Boolean;
  ...
  protected
    function GetEntityID: TEzEntityID; override;
  public
    constructor CreateEntity(const P1, P2: TEzPoint; const aNumber: String);
    destructor Destroy; override;
    procedure Initialize; override;
    procedure Draw(Grapher: TEzGrapher; Canvas: TCanvas; Const Clip: TEzRect;
      DrawMode: TEzDrawMode; Data: Pointer = Nil); override;
...
Uses
  EzBaseExpr, ezdims, EzPolyclip, EzSystem, EzGraphics, EzConsts, EzBasicCtrls, EzGisTiff;
...
Begin
  ...
  Else
    Scale := PenScale;
  APenStyle.Width := PenStyle.Width;
    ...
    Repeat
      ...
      Begin
        ...
        Begin
          ...
          TmpV.DrawOpened( Canvas, Clip, TmpV.Extension, Grapher, APenStyle, PenScale,
...
Begin
  ...
  Begin
    TmpHeight := Height;
    TmpIndex:= Index;
    if Not((Index >= 0) And (Index <= Ez_Symbols.Count-1)) then
      TmpIndex := 0;
    If TmpHeight = 0 Then
      Exit;
    Symbol := Ez_Symbols[ TmpIndex ];
    if Symbol.Count = 0 then
      Exit;
    ...
    Begin
      ...
      Begin
        ...
        If Ent.EntityID = idAlignedTTText Then
        begin
          tmp := TEzAlignedTTText( Ent ).Text;
          TEzAlignedTTText( Ent ).Text := fText;
          Found := true;
          Foundtype := Ent.EntityID;
          Break;
        end
        else
        If Ent.EntityID = idAlignedText2 Then
        begin
          tmp := TEzAlignedText2( Ent ).Text;
          TEzAlignedText2( Ent ).Text := fText;
          Found := true;
          Foundtype := Ent.EntityID;
          Break;
        end;
    ...
    If Not IsRectVisible( TmpBox, Clip ) Then
      Exit;
    ...
    Begin
      ...
      Else
      If FoundType = idFittedVectText Then
        TEzFittedVectorText( Ent ).Text := tmp
      Else
      If FoundType = idTrueTypeText Then
        TEzTrueTypeText( Ent ).Text := tmp
      Else
      If FoundType = idAlignedTTText Then
        TEzAlignedTTText( Ent ).Text := tmp
      Else
      If FoundType = idAlignedText2 Then
        TEzAlignedText2( Ent ).Text := tmp;
    End;
  End;
End;
...
Begin
  Result := FLoaded and Assigned(FBitmap);
  if not Result then
  begin
...
//      filnam := AddSlash( Ez_Preferences.CommonSubDir ) + FFileName;
      if FileExists(FFileName) then
        TheGraphicLink.ReadGeneric(FFileName);
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Center := Point(TheGraphicLink.Bitmap.Width div 2, TheGraphicLink.Bitmap.Height div 2);
          RotateBitmap(TheGraphicLink.Bitmap, FBitmap, FRotangle, Center, NewCenter);
          FLoaded := True;
        end
        else
        begin
          FBitmap := TheGraphicLink.Bitmap;
          TheGraphicLink.Bitmap := nil;
          FLoaded := True;
        end;
      end;
  end;
...
Begin
  ...
  Begin
    ...
    Write(FVisible, SizeOf(FVisible));
  End;
  //
  FOriginalSize := StorageSize;
End;

procedure TEzPictureRef.SetBitmap(const Value: TBitmap);
begin
  if FBitmap <> Value then
  begin
    UnloadBitmap;
    FBitmap := Value;
    FLoaded := Assigned(FBitmap) and (FBitmap is TBitmap);
  end;
end;

procedure TEzPictureRef.SetRotangle(Value: Double);
begin
...
procedure TEzPictureRef.SetVisible(const Value: Boolean);
begin
  FVisible := Value;
...
procedure TEzPictureRef.UnloadBitmap;
begin
  FreeAndNil(FBitmap);
  FLoaded := False;
end;
...
Begin
    ...
    Begin
      ...
      Begin
        ...
        Try
          ...
          TmpBitmap.Canvas.StretchDraw( Rect( 0, 0, TmpBitmap.Width, TmpBitmap.Height ), FBitmap);
      ...
      End
      Else
        Canvas.StretchDraw(Dest, FBitmap);
    End
    Else
    Begin
      EzGraphics.PrintBitmapEx( Canvas, Dest, FBitmap,
  ...
  Begin
    ...
    With BmpRect Do
    Begin
      fx := Grapher.DistToRealX( FBitmap.Width ) / Abs( FBox.X2 - FBox.X1 );
      fy := Grapher.DistToRealY( FBitmap.Height ) / Abs( FBox.Y2 - FBox.Y1 );
...
function TEzTrueTypeText.DoMakePolyPoints: Boolean;
begin
  Result := False;
end;
...
function TEzTrueTypeText.GetDrawTextFlags: Word;
begin
  Result := DT_NOCLIP or DT_NOPREFIX or DT_VCENTER;
end;
...
Begin
  ...
  // Если необходимо рисуем полигон под текстом
  ...
  begin
    ...
    WasCalculated := True;
    FPoints.DrawClosed(Canvas, Clip, FBox, Grapher, PenTool.FPenStyle, PenTool.Scale,
  ...
  if (DrawMode in [dmSelection, dmRubberPen]) or (GetBackgroundMode = TRANSPARENT) then
    SetBkMode(Canvas.Handle, TRANSPARENT)
  else
  begin
    SetBkMode(Canvas.Handle, GetBackgroundMode);
  ...
  begin
    if not WasCalculated and DoMakePolyPoints then
...
function TEzTrueTypeText.GetBackgroundMode: Integer;
begin
  Result := TRANSPARENT;
end;
...
function TEzTrueTypeText.IsEqualTo(Entity: TEzEntity; IncludeAttribs: Boolean): Boolean;
begin
  Result:= False;
  if Not ( Entity.EntityID = Self.EntityID ) Or
    ( Inherited IsEqualTo( Entity, IncludeAttribs ) = False )
    {$IFDEF FALSE}Or
...
constructor TEzMap500Entity.CreateEntity;
begin
...
begin
  if Assigned(FNomenclatureFunc) then
    Result := FNomenclatureFunc(FNomenclature, X, Y)
  else
    Result := False;
  if not Result then
    FNomenclature := 'Ошибка';
end;
...
class procedure TEzMap500Entity.SetNomenclatureFunc(const Value: TNomenclatureParamFunc);
begin
  FNomenclatureFunc := Value;
end;
...
begin
  Result := (FNomenclature = '') or (FNomenclature = 'Ошибка');
end;
...
begin
  try
    ...
    { Кetrieve the number of points defining the path }
    APoints := nil;
    Types := nil;
...
function TEzAlignedTTText.BasicInfoAsString: String;
begin
  Result:= Format(sATTTextInfo, [FPoints[0].X,FPoints[0].Y,FText,Ord(FAlignment),
    Fonttool.Height,RadToDeg(Fonttool.Angle)]);
end;

function TEzAlignedTTText.DoMakePolyPoints: Boolean;
begin
  Result := FAlignment <> taLeftJustify;
end;

function TEzAlignedTTText.GetDrawTextFlags: Word;
begin
...
function TEzAlignedTTText.GetEntityID: TEzEntityID;
begin
  Result := idAlignedTTText; 
end;

procedure TEzAlignedTTText.LoadFromStream(Stream: TStream);
begin
  inherited;
  Stream.Read(FAlignment, SizeOf(FAlignment));
end;

procedure TEzAlignedTTText.SaveToStream(Stream: TStream);
begin
  inherited;
  Stream.Write(FAlignment, SizeOf(FAlignment));
end;

function TEzAlignedTTText.StorageSize: Integer;
begin
  Result := inherited StorageSize;
  Result := Result + SizeOf(FAlignment);
end;

{ TEzPageEntity }

constructor TEzPageEntity.CreateEntity(const P1, P2: TEzPoint; const aNumber: String);
begin
  inherited CreateEntity(P1, P2);
  FShowNumber := True;
  FVisible := True;
...
procedure TEzPageEntity.CreateText;
var
  H: Double;
begin
  FreeAndNil(FText);
  if (Number <> '') and not IsRectEmpty2D(FBox) then
  begin
    H := (FBox.ymax - FBox.ymin) / 5;
    FText := TEzTrueTypeText.CreateEntity(Point2D(0, 0), FNumber, H, 0);
    // Смещаем в центр квадрата
    FText.BasePoint := Point2D(
      Self.FBox.xmin + (Self.FBox.xmax - Self.FBox.xmin - (FText.FBox.xmax - FText.FBox.xmin)) / 2,
      Self.FBox.ymax - H * 2
    );
  end;
end;

destructor TEzPageEntity.Destroy;
begin
  FText.Free;
  inherited;
end;

procedure TEzPageEntity.Draw(Grapher: TEzGrapher; Canvas: TCanvas;
  const Clip: TEzRect; DrawMode: TEzDrawMode; Data: Pointer);
begin
  inherited;
  if fShowNumber then
...
function TEzPageEntity.GetEntityID: TEzEntityID;
begin
  Result := idPage;
end;

procedure TEzPageEntity.Initialize;
begin
  inherited;
  CreateText;
end;

procedure TEzPageEntity.LoadFromStream(Stream: TStream);
var
  B: Boolean;
begin
  inherited;
  Number := EzReadStrFromStream(Stream);
...
procedure TEzPageEntity.SaveToStream(Stream: TStream);
begin
  inherited;
  EzWriteStrToStream(Number, Stream);
  Stream.Write(FShowNumber, SizeOf(FShowNumber));
  Stream.Write(FVisible, SizeOf(FVisible));
end;

procedure TEzPageEntity.SetNumber(const Value: String);
begin
  FNumber := Value;
  CreateText;
end;

procedure TEzPageEntity.SetShowNumber(const Value: Boolean);
begin
  FShowNumber := Value;
  CreateText;
end;

procedure TEzPageEntity.SetVisible(const Value: Boolean);
begin
  FVisible := Value;
end;
...
function TEzAlignedText2.GetBackgroundMode: Integer;
begin
  if FTransparent then
    Result := Windows.TRANSPARENT
  else
    Result := OPAQUE;
end;

function TEzAlignedText2.GetEntityID: TEzEntityID;
begin
  Result := idAlignedText2;
end;

procedure TEzAlignedText2.LoadFromStream(Stream: TStream);
begin
  inherited;
  Stream.Read(FTransparent, SizeOf(FTransparent));
end;

procedure TEzAlignedText2.SaveToStream(Stream: TStream);
begin
  inherited;
  Stream.Write(FTransparent, SizeOf(FTransparent));
end;

function TEzAlignedText2.StorageSize: Integer;
begin
  Result := inherited StorageSize;
  Result := Result + SizeOf(FTransparent);
end;
...
function TEzBitmapRef.BasicInfoAsString: string;
begin
  Result:= Format(sBmpRefInfo, [FPoints.AsString, FAlphaChannel, FRotangle]);
end;

constructor TEzBitmapRef.CreateEntity(const P1, P2: TEzPoint; Bitmap: TBitmap);
begin
  Inherited CreateEntity( P1, P2 );
  FVisible := True;
  FOriginalImage := Bitmap;
end;
...
procedure TEzBitmapRef.Draw(Grapher: TEzGrapher; Canvas: TCanvas; const Clip: TEzRect; DrawMode: TEzDrawMode;
  Data: Pointer);
var
{  GLink: TEzGraphicLink;
  Pass: Boolean;
...
Begin
  ...
  If Not IsBoxInBox2D( FBox, Clip ) Then
    Exit;
  if DrawMode = dmNormal then
    Pass := Assigned(FOriginalImage) and not FOriginalImage.Empty
  Else
    Pass := false;
    ...
    Begin
      ...
      Begin
        ...
        Try
          ...
          TmpBitmap.Width := succ( Dest.Right - Dest.Left );
          TmpBitmap.Height := succ( Dest.Bottom - Dest.Top );
          TmpBitmap.Canvas.StretchDraw( Rect( 0, 0, TmpBitmap.Width, TmpBitmap.Height ), BmpToDraw);
          bgw := BufferBitmap.Width;
          bgh := BufferBitmap.Height;
          BackgFormat := BufferBitmap.PixelFormat;
          For y := 0 To TmpBitmap.Height - 1 Do
      ...
      End
      Else
        Canvas.StretchDraw(Dest, BmpToDraw);
    End
    Else
    Begin
      EzGraphics.PrintBitmapEx( Canvas, Dest, BmpToDraw,
        Rect( 0, 0, BmpToDraw.Width, BmpToDraw.Height ) );
    End;
  End
  Else
  Begin
    ...
    With BmpRect Do
    Begin
      fx := Grapher.DistToRealX( BmpToDraw.Width ) / Abs( FBox.X2 - FBox.X1 );
      fy := Grapher.DistToRealY( BmpToDraw.Height ) / Abs( FBox.Y2 - FBox.Y1 );
      OffsetRect( Src, -Left, -Top );
    End;
    L := round( Src.Left * fx );
    T := round( Src.Top * fy );
...
function TEzBitmapRef.GetEntityID: TEzEntityID;
begin
  Result := idBitmapRef;
end;

procedure TEzBitmapRef.Initialize;
begin
  inherited;

end;

function TEzBitmapRef.IsEqualTo(Entity: TEzEntity; IncludeAttribs: Boolean): Boolean;
begin
  Result:= False;
  if Not ( Entity.EntityID = idBitmapRef )
     Or
     ( Inherited IsEqualTo( Entity, IncludeAttribs ) = False )
     {$IFDEF FALSE}
     Or
     ( IncludeAttribs And ( FFileName <> TEzPictureRef(Entity).FFileName ) )
     {$ENDIF}
  Then
    Exit;
  Result:= True;
...
procedure TEzBitmapRef.LoadFromStream(Stream: TStream);
Var
  Reserved: Array[0..41] Of byte;
Begin
  { read ID and points }
...
procedure TEzBitmapRef.MakeRotatedBitmap;
var
  Center: TPoint;
  NewCenter: TPoint;
begin
  ...
  begin
    FreeAndNil(FRotatedImage);
    FRotatedImage := TBitmap.Create;
    Center := Point(FOriginalImage.Width div 2, FOriginalImage.Height div 2);
    RotateBitmap(FOriginalImage, FRotatedImage, FRotangle, Center, NewCenter);
    FLoaded := True;
  end;
end;

procedure TEzBitmapRef.SaveToStream(Stream: TStream);
Var
  Reserved: Array[0..41] Of byte;
Begin
  Stream.Write( ID, SizeOf( ID ) );
...
procedure TEzBitmapRef.SetBitmapImage(const Value: TBitmap);
begin
  if FOriginalImage <> Value then
  begin
...
procedure TEzBitmapRef.SetRotangle(Value: Double);
begin
  if FRotangle <> Value then
    FLoaded := False;
  inherited;
...
procedure TEzBitmapRef.SetVisible(const Value: Boolean);
begin
...
function TEzBitmapRef.StorageSize: Integer;
begin
  Result := inherited StorageSize;
end;
...
constructor TEzSquareText.CreateEntity(const aCaption: String; const aTop, aLeft, aSize: Double);
begin
  FCaption := aCaption;
  FSquareSize := aSize;
  inherited CreateEntity(Point2D(aLeft, aTop), Point2D(aLeft + aSize, aTop - aSize));
  CreateText;
end;

procedure TEzSquareText.CreateText;
begin
  if Assigned(FText) then
    FreeAndNil(FText);
  FText := TEzTrueTypeText.CreateEntity(FPoints[0], FCaption, FSquareSize / 4, 0);
  // Смещаем в центр квадрата
  FText.BasePoint := Point2D(
    FBox.xmin + (FSquareSize - FText.FBox.xmax + FText.FBox.xmin) / 2,
    FBox.ymax - FSquareSize / 2 + FText.FontTool.Height / 2
  );
end;

destructor TEzSquareText.Destroy;
begin
  FText.Free;
  inherited;
end;

procedure TEzSquareText.Draw(Grapher: TEzGrapher; Canvas: TCanvas; const Clip: TEzRect; DrawMode: TEzDrawMode;
  Data: Pointer);
begin
  inherited;
  FText.Draw(Grapher, Canvas, Clip, DrawMode, Data);
...
function TEzSquareText.GetEntityID: TEzEntityID;
begin
  Result := idSquareText;
end;

function TEzSquareText.GetFontTool: TEzFontTool;
begin
...
procedure TEzSquareText.Initialize;
begin
  inherited;

end;

procedure TEzSquareText.LoadFromStream(Stream: TStream);
begin
  inherited;
  FCaption := EzReadStrFromStream(Stream);
  Stream.Read(FSquareSize, SizeOf(FSquareSize));
  FontTool.LoadFromStream(Stream);
end;

procedure TEzSquareText.SaveToStream(Stream: TStream);
begin
  inherited;
  EzWriteStrToStream(FCaption, Stream);
  Stream.Write(FSquareSize, SizeOf(FSquareSize));
  FontTool.SaveToStream(Stream);
end;
